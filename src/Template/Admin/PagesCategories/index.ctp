...
<?php
	...
	$this->append('actions', $this->Html->button(__d('me_cms', 'Add page'), ['controller' => 'Pages', 'action' => 'add'], ['class' => 'btn-success', 'icon' => 'plus']));
?>
        ...
        <?php foreach($categories as $category): ?>
            <tr>
                <td>
                    ...
                    <?php
                        ...
                        if($category->page_count) {
                            $actions[] = $this->Html->link(__d('me_cms', 'Open'), ['_name' => 'pages_category', $category->slug], ['icon' => 'external-link', 'target' => '_blank']);
                        }
                ...
                <td class="min-width text-center">
                    <?php
                        if($category->page_count) {
                            echo $this->Html->link($category->page_count, ['controller' => 'Pages', 'action' => 'index', '?' => ['category' => $category->id]], ['title' => __d('me_cms', 'View items that belong to this category')]);
                        }
                        else {
