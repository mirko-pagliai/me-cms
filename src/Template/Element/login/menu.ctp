...
<?php	
	if(!$this->request->isCurrent($url = ['_name' => 'login']))
		$menu[] = $this->Html->link(__d('me_cms', 'Login'), $url);

	//If signup is enabled
	if(config('users.signup') && !$this->request->isCurrent($url = ['_name' => 'signup']))
		$menu[] = $this->Html->link(__d('me_cms', 'Sign up'), $url);

	//If signup is enabled and if accounts will be enabled by the user via email
	if(config('users.signup') && config('users.activation') === 1 && !$this->request->isCurrent($url = ['_name' => 'resend_activation']))
		$menu[] = $this->Html->link(__d('me_cms', 'Resend activation email'), $url);

	//If reset password is enabled
	if(config('users.reset_password') && !$this->request->isCurrent($url = ['_name' => 'forgot_password']))
