...
<?php if(!empty($widgetsData['MeCms.albums'])): ?>
	<div class="widget sidebar-widget">
		<?php 
			echo $this->Html->h4(__d('me_cms', 'Albums'));
			echo $this->Form->create(FALSE, array('type' => 'get', 'url' => array('controller' => 'photos_albums', 'action' => 'view', 'plugin' => 'me_cms')));
			echo $this->Form->input('q', array(
				'empty'		=> __d('me_cms', 'Select an album'),
				'label'		=> FALSE,
				'onchange'	=> 'send_form(this)',
				'options'	=> $widgetsData['MeCms.albums']
			));
			echo $this->Form->end();
		?>
	</div>
