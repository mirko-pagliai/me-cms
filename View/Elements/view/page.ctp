	...
	<?php
		...
		if(!empty($page['Page']['created']))
			$content_info .= $this->Html->div('content-date',
				__d('me_cms', 'Posted on %s', $this->Time->format($page['Page']['created'], $config['main']['datetime']['long'])), 
				array('icon' => 'clock-o')
			);
		...
		if(!empty($page['Page']['text'])) {
			...
			if($params['action'] !== 'view' && !empty($config['frontend']['truncate_to']))
				echo $this->Html->div('content-text', $truncate = $this->Text->truncate(
					$$page['Page']['text'], $config['frontend']['truncate_to'], array('exact' => FALSE, 'html' => TRUE)
				));
			else
